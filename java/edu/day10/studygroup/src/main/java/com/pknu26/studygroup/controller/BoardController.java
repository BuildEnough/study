...
public class BoardController {
    ...
    @Autowired
    private ReplyService replyService; // 댓글 가져오기
    ...
    public String showCreateForm(@ModelAttribute("boardForm") BoardForm boardForm) {
        ...
        // #PRC02 - 입력 값 검증 BoardForm을 읽어온 후, /board/form.html에 model로 전달 -> form.html
        return "/board/form"; // html 작성 안 함, /board/form.html 화면 띄움
    }

    // 글쓰기 POST
    // #PRC04 - 폼 화면에서 저장버튼 누르면 발생
    // BoardForm 입력검증 클래스, BindingResult 검증처리
    @PostMapping("/create")
    public String create(@Valid BoardForm boardForm, BindingResult bindingResult) {

        // #PRC05 - 입력에 에러가 있으면 다시 form.html 이동
        if (bindingResult.hasErrors()) {
            return "/board/form";
        }

        // #PRC06 - 입력 값이 정확하면 서비스에 메서드 호출, 파라미터로 form 값 입력 -> BoardService.java
        boardService.createBoard(boardForm);
        // #PRC14 - 화면이 목록 화면으로 표시
    ...
    public String edit(@PathVariable("boardId") Long boardId,
        ...
        return "redirect:/board/detail/" + boardId; // 수정된 자신 글 상세로 이동
    }
