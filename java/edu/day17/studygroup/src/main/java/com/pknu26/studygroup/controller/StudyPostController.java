...
public class StudyPostController {
    ...
    private final CommentService commentService;  // 260421. 댓글 서비스 추가
    ...
    // HTML 상에서 #authentication.getPrincipal().getUsername() 사용가능
    //    LoginUser loginUser = (LoginUser) session.getAttribute("loginUser");

        if(loginUser ==  null) {
            return "redirect:/user/login";  // 로그인 안한 사람은 게시판 글 못 씀
        }
    ...
    public String create(@Valid StudyPostForm studyPostForm, BindingResult bindingResult, Model model, @AuthenticationPrincipal CustomUserDetails loginUser) {
        ...
        studyPostForm.setUserId(loginUser.getUserId());  // 세션에 로그인한 사용자 아이디
...
// 댓글 기능 추가
    ...
    public String detail(@PathVariable("postId") Long postId, Model model, @AuthenticationPrincipal CustomUserDetails loginUser) {
        // LoginUser loginUser = (LoginUser) session.getAttribute("loginUser");
        ...
        model.addAttribute("canReq", canReq);  // 신청가능여부
