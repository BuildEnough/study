...
public class BoardController {
    ...
    public String list(@ModelAttribute PageRequest pageRequest, Model model) {
        ...
        if (loginUser != null) {
            // 사용자와 아이디가 같으면 isWriter 은 true
            boolean isWriter = board.getWriter().equals(loginUser.getLoginId());
            // 사용자의 권한이 ROLE_ADMIN이면 isAdmin 은 true
    ...
    // public String showCreateForm(Model model, HttpSession session) { 에서 SpringSecurity로 변경
    ...
    public String showEditForm(@PathVariable("boardId") Long boardId, Model model, @AuthenticationPrincipal CustomUserDetails loginUser) throws Exception {
        Board board = this.boardService.readBoardById(boardId);
