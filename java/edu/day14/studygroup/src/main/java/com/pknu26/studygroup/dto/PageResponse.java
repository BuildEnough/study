...
public class PageResponse<T> {
    ...
    private final int totalPage;     // 전체 페이지수, 게시글이 32/10 = 3.2를 올림해서 4페이지 필요
    ...
    */
    ...
    private final boolean prev; // 이전 버튼 표시 여부
    private final boolean next; // 끝버튼 표시여부
    ...
    public PageResponse(List<T> dtoList, int totalCount, int currentPage, int size) {
        ...
        // 현재 페이지 번호 묶음 끝보다 전체 페이지 수가 크면, 뒤에 다른 페이지 묶음이 존재한다
