...
public class BoardController {
    ...
    public String showCreate(Model model, BoardForm boardForm) {
        model.addAttribute("mode", "create"); // 같은 화면의 POST 액션을 분리
    ...
    public String showModify(Model model, BoardForm boardForm, @PathVariable("bno") Long bno) {
        ...
        boardForm.setTitle(board.getTitle()); // 실제 DB에서 넣어온 board 데이터를 입력용 BoardForm 클래스 멤버변수로 할당
    ...
    public String modifyBno(@Valid BoardForm boardForm, BindingResult bindingResult, @PathVariable("bno") Long bno) {
        if (bindingResult.hasErrors()) {
            return "board_create"; //
        }

        // boardService에 수정용 메서드 추가
        Board board = this.boardService.getBoardOne(bno);
