...
### 사용자, 권한, 롤
- DB용어
    - 스키마: DB객체, 사용자, 제약조건 등을 그룹으로 관리하는 단위, 타 DB에서는 일반적으로 데이터베이스
    - 오라클에서는 사용자를 생성하면 사용자명과 동일한 스키마가 만들어짐
- 사용자 생성- [쿼리](./code/day07/1.사용자_권한.sql)
    ```sql
...
- 사용자 권한
    ...
    - USER, SESSION, TABLE, INDEX, VIEW, SEQUENCE, ... 등 객체 별로 생성, 변경, 삭제 등 권한을 각각 부여가능
    - 특히 TABLE은 INSERT, SELECT, UPDATE, DELETE의 DML 권한도 따로 부여 가능

    ```sql
    -- 권한 부여
    GRANT 시스템 권한 TO 사용자;
    [WITH ADMIN OPTION]; -- 사용자가 받은 권한을 다른 사용자에게 부여할 수 있는 옵션

    -- 권한 해제
    REVOKE 시스템권한 FROM 사용자;
    ```
    - 새로 생성한 사용자에게 아무런 권한도 부여하지 않으면 접속 불가  
    ![alt text](img/10.png)
- 롤
    - 사용자 권한 종류를 각 객체별로 전부 지정하면 너무 많은 지정이 필요
    - 여러 권한을 한 꺼번에 가지는 객체 생성한 것이 롤
...
### PL/SQL
- 개요
    - SQL 문만 사용해서 해결하기 어려운 작업들이 존재
    - 프로그래밍 기법 사용해서 문제를 해결 -> PL/SQL
    - 모든 DB가 프로그래밍이 가능
    - PL/SQL 이라는 용어는 Oralce에서만 사용
- PL/SQL- [쿼리](./code/day07/3.plsql_first.sql)
...
- 변수 선언
    ...
    DECLARE
        변수명 변수타입 [:= 값할당];
    ```
- 조건 제어문- [쿼리](./code/day07/4.IF문.sql)
    ```sql
    -- IF 문
    IF 조건식1 THEN
        수행할 명령어;
...
- 반복문- [쿼리](./code/day07/5.반복문.sql)
    ...
    END LOOP;
    ```
- 예외처리- [쿼리](./code/day07/6.예외처리.sql)
    - 오류: 코드 상 문법 상 오류, Error
    - 예외: 실행 중 발생하는 오류, Exception  
    - RAISE: 예외 생성
    ![alt text](img/11.png)
...
- 종류
    - 저장 프로시저: 특정 처리 작업 수행을 위한 서브프로그램으로 SQL문 상에서 사용불가. 스케줄과 같은 큰 배치 작업에 활용
    - 함수: 특정 연산 결과를 반환하는 서브 프로그램. SQL문에서 사용가능. 내장함수와 동일
    - 패키지: 저장 서브프로그램의 그룹화
    - 트리거: DB의 강력한 기능. 특정 상황 발생 시 자동으로 연달아 수행하는 기능 구현
- 저장 프로시저- [쿼리](./code/day07/8.저장서브프로그램1.sql)
    - 보통 프로시저로 호칭
    - DBEAVER에서는 스키마 procedures 폴더에서 Create New Procedure 메뉴로 생성
    ```sql
    -- 기본 문법
    ...
    CALL 프로시저명([파라미터]);
    ```
- 함수- [쿼리](./code/day07/11.함수.sql)
    - 파이썬 return 있는 함수와 동일, 다른 SQL문에서 사용가능
    - 내장함수, UPPER(), LOWER(), INSTR(), SYSDATE 등에 원하는 기능이 없으면 직접 함수를 구현해야함  
    ![](img/12.png)
...
- 패키지
    ...
    ```
- 트리거- [쿼리](./code/day07/13.트리거생성.sql)
...
- 종류
    ...
    - MySQL Workbench: 논리/물리적 다이어그램 생성 가능, 생성 그대로 DB가 됨

## Day09
### 데이터베이스 모델링
#### 요구사항 분석
- DB설계자와 업무 담당자(병원관계자)와 협의
    1. 의사관리, 간호사관리, 환자관리, 진료과관리, 입원관리, 진료처리
        - `의사`, `간호사`, `환자`, `진료과` -> 명사
        - `입원`, `진료` -> 동사
    2. 의사: 고유`의사번호`, `이름`, `진료과목`, `주소`, `전화번호`, `주민번호`, `직급`, `의사약력`, `과번호` ...
    3. 간호사: 고유간호사번호, 이름, 진료과목, 주소, 전화번호, 주민번호, 직급, 과번호 ...
    4. 환자: 고유환자번호, 이름, 주민번호, 전화번호, 주소, 키, 몸무게, 혈액형, 성별, 진료과, ...
    5. 입원: 입원일, 퇴원일, 입원실정보
    6. 진료과: 고유의과번호, 과이름, 전화번호, 팩스번호, ...
    7. 의사는 여러 명의 환자를 진찰하고, 환자도 여러 명의 의사에게 진료를 받을 수 있음
    8. 진료를 받으면 고유진료번호, 진료일이 저장
    9. 간호사는 여러 명의 환자를 처치할 수 있고, 처치일, 처치내용 등이 저장
    10. 진료내용은 여러 값이 저장됨
...
- 논리설계 이전 기본키/외래키, 일반속성 정립
    ...
    - 간호사: `간호사번호`, 이름, ~~진료과목~~, 주소, 전화번호, ~~주민번호~~, 직급, _과번호_
    - 환자: `환자번호`, 이름, 주민번호, 전화번호, 주소, 키, 몸무게, 혈액형, 성별, **진료과**, **처치일**, **처치내용**
    - 입원: `입원번호`, 환자번호, 입원일, 퇴원일, 병실
- 도메인
    - 특정 컬럼에 들어가는 값의 범위
    - 초등학교 1~6, 대학교 1~4학년
- DB설계는 일회성이 아니라, 중간에 계속 변경가능


#### ERDCLOUD 사용법 및 모델링
- 회원가입 후 로그인
1. ERD 생성 클릭
    - 제목(병원업무관리 시스템), 태그(모델링 연습)
2. 새로운 엔티티 추가
    - 노란색 +: 키 추가
    - 하늘색 +: 속성 추가  
    ![alt text](img/18.png)
3. 관계(없거나 한 개 또는 여러 개) 클릭
    - 부모 테이블 클릭 후 자식 테이블 클릭
    - 식별관계 비식별관계 선택
...
4. 각 엔티티와 키와 속성 관계로 논리 다이어그램 작성  
    ![alt text](img/21.png)
5. 물리 다이어그램으로 변경 작성, 각
    ![alt text](img/22.png)
6. 내보내기 - [쿼리](./code/day09/3.병원물리스키마_비식별관계.sql)
    - DB 종류 선택: 현재 Oracle
    - PK 제약조건, FK 제약조건, 비식별 제약조건 추가 선택
...
#### 모델링 시 주의점
- 식별관계로 모델링을 하면 JOIN할 때 많은 키(PK/FK)를 조인절에 포함해서 복잡
- ERDCloud에서 식별관계 SQL 쿼리를 제대로 생성 못함
...
### DBeaver 팁
윈도우->옵션->환경설정->편집기->SQL 편집기->SQL 자동완성->enable auto activation
...
#### 테스트 실행 순서
- 주문 테이블 생성: ORDERS_BIG - [쿼리](./code/day10/1.인덱스테스트용_테이블.sql)
- 300만 건
- 인덱스 테스트 - [쿼리](./code/day10/2.300만건_더미데이터생성.sql)
